...
static void print_sensor_gps_message(const struct orb_metadata* meta, const void* buffer)
{
    ...
    uorbinfo_raw("%s:\ttimestamp: %" PRIu64 " (%" PRIu64 " us ago) time_utc: %" PRIu64 " latitude: %.4f longitude: %.4f",
        meta->o_name, message->timestamp, now - message->timestamp, message->time_utc, message->latitude, message->longitude);
...
{
    const struct sensor_gps_satellite* message = (const struct sensor_gps_satellite*)buffer;
    const orb_abstime now = orb_absolute_time();
    int i;
    ...
    for (i = 0; i < message->count; i++) {
        uorbinfo_raw("%s:\tnumber:%d svid: %d .elevation: %d azimuth: %d snr: %d",
...
ORB_DEFINE(sensor_gps_satellite, struct sensor_gps_satellite, print_sensor_gps_satellite_message, sensor_gps_satellite);
#else
ORB_DEFINE(sensor_gps, struct sensor_gps, NULL, sensor_gps);
ORB_DEFINE(sensor_gps_satellite, struct sensor_gps_satellite, NULL, sensor_gps_satellite);
#endif
